...
class SeparableConv2d_ghost(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, padding=0, dilation=1, bias=False):
        super(SeparableConv2d_ghost, self).__init__()
...
def fixed_padding_ghost(inputs, kernel_size, rate):
...
class SeparableConv2d_same_ghost(nn.Module):
    def __init__(self, inplanes, planes, kernel_size=3, stride=1, dilation=1, bias=False):
        super(SeparableConv2d_same_ghost, self).__init__()
#         self.ghost = GhostModule(inplanes, planes, kernel_size=3, stride=stride, dilation=1, relu=False, batchnorm=False)
    ...
    def forward(self, x):
        x = fixed_padding_ghost(x, self.conv1.kernel_size[0], rate=self.conv1.dilation[0])
#         x = self.ghost(x)
        x = self.conv1(x)
        x = self.pointwise(x)
        return x
...
class Block(nn.Module):
    def __init__(self, inplanes, planes, reps, stride=1, dilation=1, start_with_relu=True, grow_first=True, is_last=False):
        ...
        if planes != inplanes or stride != 1:
            self.skip = GhostModule_xcept(inplanes, planes, kernel_size=1, stride=stride, relu=False)
...
#             rep.append(GhostModule(inplanes, planes, kernel_size=3, stride=1, relu=False, dilation=dilation))
            rep.append(SeparableConv2d_same_ghost(inplanes, planes, 3, stride=1, dilation=dilation))
...
#             rep.append(GhostModule(filters, filters, kernel_size=3, stride=1, relu=False, dilation=dilation))
            rep.append(SeparableConv2d_same_ghost(filters, filters, 3, stride=1, dilation=dilation))
...
#             rep.append(GhostModule(inplanes, planes, kernel_size=3, stride=1, relu=False, dilation=dilation))
        ...
        if stride != 1:
            rep.append(GhostModule_xcept(planes, planes, kernel_size=3, stride=2, relu=False, batchnorm=False))
#             rep.append(SeparableConv2d_same(planes, planes, 3, stride=2))

        if stride == 1 and is_last:
            rep.append(GhostModule_xcept(planes, planes, kernel_size=3, stride=1, relu=False, batchnorm=False))
...
class Xception_ghost(nn.Module):
    ...
    def __init__(self, inplanes=3, os=16, pretrained=False):
        ...
        # Entry flow
        self.conv1 = GhostModule_xcept(inplanes, 32, kernel_size=3, stride=2)
#         self.conv1 = nn.Conv2d(inplanes, 32, 3, stride=2, padding=1, bias=False)
#         self.bn1 = nn.BatchNorm2d(32)
        self.relu = nn.ReLU(inplace=True)

        self.conv2 = GhostModule_xcept(32, 64, kernel_size=3, stride=1, relu=False)
...
#         self.bn2 = nn.BatchNorm2d(64)
        ...
        self.conv3 = SeparableConv2d_same_ghost(1024, 1536, 3, stride=1, dilation=exit_block_rates[1])
        self.bn3 = nn.BatchNorm2d(1536)
        
        self.conv4 = SeparableConv2d_same_ghost(1536, 1536, 3, stride=1, dilation=exit_block_rates[1])
        self.bn4 = nn.BatchNorm2d(1536)

        self.conv5 = SeparableConv2d_same_ghost(1536, 2048, 3, stride=1, dilation=exit_block_rates[1])
...
class DeepLabv3_plus_xcept_ghost(nn.Module):
    def __init__(self, nInputChannels=3, n_classes=21, os=16, pretrained=False, _print=True):
        ...
        super(DeepLabv3_plus_xcept_ghost, self).__init__()

        # Atrous Conv
        self.xception_features = Xception_ghost(nInputChannels, os, pretrained)
        ...
        self.global_avg_pool = nn.Sequential(nn.AdaptiveAvgPool2d((1, 1)),
                                             GhostModule_xcept(2048, 256, kernel_size=1, stride=1)
#                                              nn.Conv2d(2048, 256, 1, stride=1, bias=False),
#                                              nn.BatchNorm2d(256),
#                                              nn.ReLU()
                                            )
        self.conv1 = GhostModule_xcept(1280, 256, kernel_size=1, stride=1)
#         self.conv1 = nn.Conv2d(1280, 256, 1, bias=False)
#         self.bn1 = nn.BatchNorm2d(256)

        # adopt [1x1, 48] for channel reduction.
        self.conv2 = GhostModule_xcept(128, 48, kernel_size=1, stride=1)
#         self.conv2 = nn.Conv2d(128, 48, 1, bias=False)
#         self.bn2 = nn.BatchNorm2d(48)

        self.last_conv = nn.Sequential(GhostModule_xcept(304, 256, kernel_size=3, stride=1),
                                       GhostModule_xcept(256, 256, kernel_size=3, stride=1),
                                       GhostModule_xcept(256, n_classes, kernel_size=1, stride=1, relu=False, batchnorm=False)
...
if __name__=="__main__":
    ...
    net = DeepLabv3_plus_xcept_ghost().eval()
